      ...
      public:
        ...
        auto static setup_member_functions(const LuaMadeSimple::Lua::Table& table) -> void
        {
            ...
            table.add_pair("GetOuter", [](const LuaMadeSimple::Lua& lua) -> int {
                ...
                if (!remote) { lua.throw_error("GetOuter: object has null remote pointer"); }
            ...
            table.add_pair("IsAnyClass", [](const LuaMadeSimple::Lua& lua) -> int {
                const auto& lua_object = lua.get_userdata<SelfType>();
                auto* remote = lua_object.get_remote_cpp_object();
                if (!remote) { lua.throw_error("IsAnyClass: object has null remote pointer"); }
            ...
            table.add_pair("IsClass", [](const LuaMadeSimple::Lua& lua) -> int {
                ...
                const auto& lua_object = lua.get_userdata<SelfType>();
                auto* remote = lua_object.get_remote_cpp_object();
                if (!remote) { lua.throw_error("IsClass: object has null remote pointer"); }
                lua.set_bool(remote->template IsA<Unreal::UClass>());
                return 1;
            });
            ...
            table.add_pair("GetWorld", [](const LuaMadeSimple::Lua& lua) -> int {
                const auto& lua_object = lua.get_userdata<SelfType>();
                auto* remote = lua_object.get_remote_cpp_object();
                if (!remote) { lua.throw_error("GetWorld: object has null remote pointer"); }
...
#1: HasAllFlags(EObjectFlags ObjectFlags))"};
                ...
                auto* remote = lua_object.get_remote_cpp_object();
                if (!remote) { lua.throw_error("HasAllFlags: object has null remote pointer"); }
                lua.set_bool(remote->HasAllFlags(object_flags));
                return 1;
            });
...
#1: HasAnyFlags(EObjectFlags ObjectFlags))"};
                ...
                Unreal::EObjectFlags object_flags = static_cast<Unreal::EObjectFlags>(lua.get_integer());
                auto* remote = lua_object.get_remote_cpp_object();
                if (!remote) { lua.throw_error("HasAnyFlags: object has null remote pointer"); }
                lua.set_bool(remote->HasAnyFlags(object_flags));
                return 1;
            });
...
#1: HasAnyInternalFlags(EInternalObjectFlags InternalObjectFlags))"};
                ...
                auto* remote = lua_object.get_remote_cpp_object();
                if (!remote) { lua.throw_error("HasAnyInternalFlags: object has null remote pointer"); }
                lua.set_bool(remote->HasAnyInternalFlags(object_internal_flags));
                return 1;
            });
...
#1: ProcessConsoleExec(string Cmd, nil Reserved, UObject Executor))"};
                ...
                auto* remote = lua_object.get_remote_cpp_object();
                if (!remote) { lua.throw_error("ProcessConsoleExec: object has null remote pointer"); }
                auto ar = Unreal::FOutputDevice{};
                auto return_value =
                        remote->ProcessConsoleExec(FromCharTypePtr<TCHAR>(cmd.c_str()), ar, executor.get_remote_cpp_object());
      ...
      private:
        auto static prepare_to_handle(const Operation operation, const LuaMadeSimple::Lua& lua) -> void
        {
            ...
            if (!field)
            {
                ...
                if (!object)
                {
                    lua.throw_error("[VeinCF] prepare_to_handle: object is null");
                }
                ...
                if (!obj_as_struct)
                {
                    ...
                    if (!cls)
                    {
                        lua.throw_error("[VeinCF] prepare_to_handle: object has null ClassPrivate");
                    }
                    obj_as_struct = cls;
                }
