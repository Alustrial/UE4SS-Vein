...
namespace RC::LuaType
{
    ...
    auto call_ufunction_from_lua(const LuaMadeSimple::Lua& lua) -> int
    {
        ...
        {
            lua.throw_error("call_ufunction_from_lua: UObject is null");
        }
    ...
    auto construct_fname(const LuaMadeSimple::Lua& lua) -> void
    {
        ...
        if (!remote)
        {
            lua.throw_error("construct_fname: object has null remote pointer");
        }
    ...
    auto construct_uclass(const LuaMadeSimple::Lua& lua) -> void
    {
        ...
        if (!remote)
        {
            lua.throw_error("construct_uclass: object has null remote pointer");
        }
    ...
    auto push_arrayproperty(const PusherParams& params) -> void
    {
        ...
        auto lua_to_memory = [&]() {
            if (params.lua.is_userdata(params.stored_at_index))
            {
                ...
                if (!source_array)
                {
                    params.throw_error("push_arrayproperty", "Source TArray is null");
                }
    ...
    auto push_setproperty(const PusherParams& params) -> void
    {
        ...
        auto lua_to_memory = [&]() {
            if (params.lua.is_userdata(params.stored_at_index))
            {
                ...
                if (!source_set) { params.throw_error("push_setproperty", "Source TSet is null"); }
    ...
    auto push_mapproperty(const PusherParams& params) -> void
    {
        ...
        case Operation::Set: {
            if (params.lua.is_userdata(params.stored_at_index))
            {
                ...
                if (!source_map) { params.throw_error("push_mapproperty", "Source TMap is null"); }
    ...
    // Check if a pointer is safely readable. Uses IsBadReadPtr (Windows API).
    // Returns false for null or garbage pointers.
    static bool is_pointer_readable(const void* ptr, size_t size = 8)
    {
        if (!ptr) return false;
    ...
    auto push_textproperty(const PusherParams& params) -> void
    {
        ...
        case Operation::GetNonTrivialLocal:
        case Operation::Get: {
            // Guard against uninitialized FText (e.g. widgets from StaticConstructObject).
            // Data may be garbage (non-null but unreadable), probe before touching.
            if (!text->Data || !is_pointer_readable(text->Data))
            {
                Unreal::FText empty{};
                LuaType::FText::construct(params.lua, empty);
            }
            else
            {
        ...
        case Operation::Set: {
            ...
            {
                // Support setting TextProperty from a plain Lua string
                auto str_val = params.lua.get_string(params.stored_at_index);
                std::wstring wide_str(str_val.begin(), str_val.end());
                text->SetString(Unreal::FString(wide_str.c_str()));
            }
            else if (params.lua.is_userdata(params.stored_at_index))
            {
                ...
                auto& src_ref = lua_other_object.get_local_cpp_object();
                if (src_ref.Data && is_pointer_readable(src_ref.Data))
                {
                    text->SetString(std::move(src_ref.ToFString()));
                }
                else
                {
                    Unreal::FText empty_text(STR(""));
                    text->SetString(std::move(empty_text.ToFString()));
                }
            }
            else
            {
                params.throw_error("push_textproperty", "Set requires FText userdata or string");
            }
            return;
        }
        case Operation::GetParam:
