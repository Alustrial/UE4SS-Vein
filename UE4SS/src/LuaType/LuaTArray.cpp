...
namespace RC::LuaType
{
    ...
    auto TArray::setup_metamethods(BaseObject& base_object) -> void
    {
        ...
        base_object.get_metamethods().create(LuaMadeSimple::Lua::MetaMethod::Length, [](const LuaMadeSimple::Lua& lua) -> int {
            ...
            if (!arr) { lua.throw_error("TArray length: array is null"); }
            lua.set_integer(arr->Num());
            return 1;
        });
    }
    ...
    auto TArray::setup_member_functions(const LuaMadeSimple::Lua::Table& table) -> void
    {
        ...
        table.add_pair("GetArrayNum", [](const LuaMadeSimple::Lua& lua) -> int {
            auto& lua_object = lua.get_userdata<TArray>();
            auto* arr = lua_object.get_remote_cpp_object();
            if (!arr) { lua.throw_error("TArray:GetArrayNum: array is null"); }
            lua.set_integer(arr->Num());
            return 1;
        });
        ...
        table.add_pair("GetArrayMax", [](const LuaMadeSimple::Lua& lua) -> int {
            auto& lua_object = lua.get_userdata<TArray>();
            auto* arr = lua_object.get_remote_cpp_object();
            if (!arr) { lua.throw_error("TArray:GetArrayMax: array is null"); }
            lua.set_integer(arr->Max());
            return 1;
        });
        ...
        table.add_pair("GetArrayDataAddress", [](const LuaMadeSimple::Lua& lua) -> int {
            auto& lua_object = lua.get_userdata<TArray>();
            auto* arr = lua_object.get_remote_cpp_object();
            if (!arr) { lua.throw_error("TArray:GetArrayDataAddress: array is null"); }
            lua.set_integer(reinterpret_cast<uintptr_t>(arr->GetData()));
            return 1;
        });
        ...
        table.add_pair("Empty", [](const LuaMadeSimple::Lua& lua) -> int {
            auto& lua_object = lua.get_userdata<TArray>();
            auto* arr = lua_object.get_remote_cpp_object();
            if (!arr) { lua.throw_error("TArray:Empty: array is null"); }
            arr->Empty(0, lua_object.m_inner_property->GetElementSize(), lua_object.m_inner_property->GetMinAlignment());
            return 0;
        });
        ...
        table.add_pair("ForEach", [](const LuaMadeSimple::Lua& lua) -> int {
            ...
            {
                auto* arr = lua_object.get_remote_cpp_object();
                if (!arr) { lua.throw_error("TArray:ForEach: array is null"); }
                uint8_t* array_data = static_cast<uint8_t*>(arr->GetData());
                if (!array_data && arr->Num() > 0) { lua.throw_error("TArray:ForEach: array data is null but Num > 0"); }

                int32_t array_size = arr->Num();
    ...
    {
        if (!array) { lua.throw_error("TArray::handle_unreal_property_value: array is null"); }
        ...
        if (StaticState::m_property_value_pushers.contains(name_comparison_index))
        {
            ...
            if (!array_data) { lua.throw_error("TArray::handle_unreal_property_value: array data is null"); }
    ...
    auto TArray::prepare_to_handle(const LuaMadeSimple::Type::Operation operation, const LuaMadeSimple::Lua& lua) -> void
    {
        ...
        if (!array) { lua.throw_error("TArray: array is null in prepare_to_handle"); }
        auto num = array->Num();
